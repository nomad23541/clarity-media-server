...
module.exports.getSeasonDetails = function(params) {
    return new Promise(function(resolve, reject) {
        let url = '/3/tv/' + params.tvid + '/season/' + params.seasonNumber + '?apiKey=' + params.apiKey
        requestLimit(url, function(err, res, body) {
            if(err) return reject(err)
            resolve(JSON.parse(body))
        })
...
module.exports.getEpisodeDetails = function(params) {
    return new Promise(function(resolve, reject) {
        let url = '/3/tv/' + params.tvid + '/season/' + params.seasonNumber + '/episode/' + params.episodeNumber + '?apiKey=' + params.apiKey
        requestLimit(url, function(err, res, body) {
            if(err) return reject(err)
            resolve(JSON.parse(body))
        })
