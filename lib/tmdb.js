...
module.exports.getShowDetails = function(params) {
    return new Promise(function(resolve, reject) {
        let url = '/3/tv/' + params.tvid + '?api_key=' + params.apiKey
        requestLimit(url, function(err, res, body) {
            if(err) return reject(err)
            resolve(JSON.parse(body))
        })
...
// params = tvid, season number, and api key
module.exports.getSeasonDetails = function(params) {
    return new Promise(function(resolve, reject) {
        let url = '/3/tv/' + params.tvid + '/season/' + params.seasonNumber + '?api_key=' + params.apiKey
        requestLimit(url, function(err, res, body) {
            if(err) return reject(err)
            resolve(JSON.parse(body))
        })
...
// params = tvid, season number, episode number, and api key
module.exports.getEpisodeDetails = function(params) {
    return new Promise(function(resolve, reject) {
        let url = '/3/tv/' + params.tvid + '/season/' + params.seasonNumber + '/episode/' + params.episodeNumber + '?api_key=' + params.apiKey
        requestLimit(url, function(err, res, body) {
            if(err) return reject(err)
            resolve(JSON.parse(body))
        })
